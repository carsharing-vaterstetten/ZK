...
void Modem::firmwareCheckAndUpdateIfNeeded()
{
    ...
    SerialMon.println("MAC: " + macStr);
    String body = "{\"mac_address\":\"" + macStr + "\",\"firmware_version\":\"" FIRMWARE_VERSION "\"}";
    SerialMon.println("Body: " + body);
    int err = sendRequest("/firmware/", "POST", body);
    if (err != 0)
    {
